$.getJSON("app.php?action=index&format=json"+(getVars['ID']!=undefined ? '&ID='+getVars['ID']: ''),function(data){
	...
	var template='<table class="table table-striped"><thead><tr><th scope="col">ID</th><th scope="col">Type</th><th scope="col">Name</th><th scope="col">Actions</th></tr></thead>{{#shapes}}<tr><td>{{ID}}</td><td>{{type}}</td><td>{{n}}</td><td><a class="btn btn-outline-dark btn-xs" href="app.php?ID={{ID}}">edit</a> <button type="button" class="btn btn-outline-danger btn-xs" data-id="{{ID}}">delete</button> </td></tr>{{/shapes}}</table>';
